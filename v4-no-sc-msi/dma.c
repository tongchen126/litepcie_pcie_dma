...
#define LITEPCIE_DMA_BUF_ORDER (10)
...
static void irq0_work_func(struct work_struct *work)
{
	...
	litepcie_writel(s,CSR_PCIE_HOST_PCIE2WB_DMA_BUS_ADDR_ADDR, 0);
...
static void irq1_work_func(struct work_struct *work)
{
	...
	litepcie_writel(s,CSR_PCIE_HOST_WB2PCIE_DMA_BUS_ADDR_ADDR, 0 + LITEPCIE_DMA_BUF_SIZE);
...
static irqreturn_t litepcie_interrupt(int irq, void *data)
{
	...
	pci_info(s->dev,"litepcie: interrupt irq_enable 0x%x, irq_vector 0x%x, irq %d, dma data 0x%x\n",irq_enable,irq_vector,irq, *(int *)s->host_dma_addr);
	//irq_vector &= irq_enable;
	if ((irq_vector & (1 << IRQ1_INTERRUPT)) && !work_busy(&s->irq0_work))
		schedule_work(&s->irq0_work);
	if ((irq_vector & (1 << IRQ2_INTERRUPT)) && !work_busy(&s->irq1_work))
		schedule_work(&s->irq1_work);
	
	litepcie_writel(s, CSR_PCIE_MSI_CLEAR_ADDR, irq_vector);
...
static int litepcie_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	*/
	...
	litepcie_writel(litepcie_dev, CSR_PCIE_MSI_ENABLE_ADDR, 0xffffffff);
